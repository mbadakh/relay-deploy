...
Public-image publishing requires the narrowly scoped OIDC role described in `ci/public-publisher-iam.json`; the role is installed and `PUBLIC_ECR_PUBLISH_ROLE_ARN` is configured in both publisher repositories. The AWS token-exchange permission follows the approved correction; image uploads remain restricted to six repositories and OIDC trust to the two main branches. No AWS access key or fallback to the runner's ambient credentials is used. `registry.r3l4y.dev` is an image catalog; Docker pulls use the actual `public.ecr.aws` names because a DNS CNAME alone cannot provide a valid custom Docker registry endpoint.
